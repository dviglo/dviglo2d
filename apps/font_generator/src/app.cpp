...
void App::show_ui()
{
...

    {
        // Рендерим в текстуру
        if (!fbo_)
            fbo_ = make_unique<Fbo>(ivec2(1024, 1024));

        if (generated_font_)
        {
            static StrUtf8 test_text = "Съешь ещё этих мягких французских булок, да выпей чаю. 1234567890";
            static bool shadow = false;
            static ImVec4 back_color(0.f, 0.f, 0.f, 0.f);
            static vec2 pos(4.f, 1.f);
            static vec2 shadow_offet(1.f, 1.f);
            ...
            if (shadow)
            {
                ...
                sprite_batch_->draw_string(test_text, generated_font_.get(), shadow_pos, 0xFF000000);
            }

            sprite_batch_->draw_string(test_text, generated_font_.get(), pos, 0xFFFFFFFF);
            sprite_batch_->flush();
            ...
            PushItemWidth(-FLT_MIN); // Label элементов не используем
            TextUnformatted("Текст");
            SameLine();
            InputText("##test_text", &test_text);

            TextUnformatted("Позиция текста");
            SameLine();
            InputFloat2("##text_pos1", &pos[0]);

            //SameLine();
            TextUnformatted("Тень");
            SameLine();
            Checkbox("##test_text_shadow", &shadow);
            SameLine();
            TextUnformatted("Смещение тени");
            SameLine();
            InputFloat2("##shaw_off", &shadow_offet[0]);

            TextUnformatted("Цвет фона");
            SameLine();
            ColorEdit4("##test_text_image_back_color", (f32*)&back_color, ImGuiColorEditFlags_AlphaPreview);

            PopItemWidth(); // Label элементов не используем

            {
                BeginChild("##test_text_image", ImVec2(0, 0), ImGuiChildFlags_Borders, ImGuiWindowFlags_HorizontalScrollbar);
