...
void App::show_ui()
{
    ...
    static GeneratedFont generated_font{};
    ...
    // Окно настройки шрифта
    {
        ...
        // Кнопка генерации
        {
            ...
            {
                generated_font = generate_font_simple(font_settings);
                ref_clamp(current_page, 0, (i32)generated_font.pages.size() - 1);
                glyph_texture = Texture(generated_font.pages[current_page]);
            }
        }
    ...
    // Окно вывода текстур
    {
        ...
        if (generated_font.pages.size() > 0)
        {
            if (Button("<"))
            {
                --current_page;
