...
SpriteFont::SpriteFont(const StrUtf8& file_path)
{
    ...
    if (!result)
    {
        DV_LOG->write_error(format("SpriteFont::SpriteFont(\"{}\") | !result", file_path));
        return;
    }
    ...
    {
        DV_LOG->write_error(format("SpriteFont::SpriteFont(\"{}\") | root_node.name() != string(\"font\")", file_path));
        return;
    }
    ...
    if (!pages_node)
    {
        DV_LOG->write_error(format("SpriteFont::SpriteFont(\"{}\") | !pages_node", file_path));
        return;
    }
    ...
    for (i32 i = 0; i < pages; ++i)
    {
        if (!page_node)
        {
            DV_LOG->write_error(format("SpriteFont::SpriteFont(\"{}\") | !page_node", file_path));
            return;
        }
...
{
    unique_ptr<Image> grayscale_image;
...
RenderedGlyph render_glyph_simpe(FT_Face face, const SFSettingsSimple& font_settings)
{
    ...
    if (font_settings.blur_radius > 0)
    {
        ...
        unique_ptr<Image> new_image = make_unique<Image>(ret.grayscale_image->size() + font_settings.blur_radius * 2,
                                                         ret.grayscale_image->num_components());

        // Вставляем в центр расширенного изображения
        new_image->paste(*ret.grayscale_image, ivec2(font_settings.blur_radius, font_settings.blur_radius));

        // TODO: Изображение может быть нулевого размера
        new_image->blur_triangle(font_settings.blur_radius);

        ret.grayscale_image = std::move(new_image);
...
SpriteFont::SpriteFont(const SFSettingsSimple& settings)
{
    ...
    while (glyph_index != 0)
    {
        ...
        stbrp_rect r{};
        r.id = vec_ind;
        r.w = rendered_glyph.grayscale_image->width() + padding * 2;
        r.h = rendered_glyph.grayscale_image->height() + padding * 2;
        rects.push_back(r);
    ...
    while (rects.size())
    {
        ...
        for (size_t i = 0; i < rects.size();)
        {
            ...
            if (rect.was_packed)
            {
                current_page->paste(*rendered_glyphs[rect.id].grayscale_image, ivec2(rect.x + padding, rect.y + padding));
                ...
                assert(rendered_glyphs[rect.id].rect.size.x == rendered_glyphs[rect.id].grayscale_image->size().x);
    ...
    for (shared_ptr<Image> page : pages)
    {
        ...
        Texture* page_tex = new Texture(colored_page, true);
    ...
    DV_LOG->write_info(format("SpriteFont::SpriteFont(const FontSettingsSimple&) | {} | Generated im {} ms", settings.src_path, duration_ms));
}
